...
exports.updateHelpCenterAnnouncement = async (req, res) => {
    ...
    try {
        ...
        if (!isEmpty(req.body.model_name)) {
            ...
            if (isEmpty(anResult)) {
                return res.status(400).json({error_msg: `400`, error_code: "model_name is empty"});
            } else {
        ...
        if (isEmpty(req.body.announcementID)) {
            return res.status(400).json({error_msg: `400`, error_code: "announcementID can not be empty"});
        }
...
exports.addModel = async (req, res) => {
    try {
        if (isEmpty(req.body.model_name)) {
            return res.status(400).json({error_msg: `400`, error_code: "model_name can not be empty"});
        }
        ...
        if (err.message.toString().search(`duplicate key error`) !== 0) {
            ...
            return res.status(400).json({error_msg: `400`, error_code: "model_name can not be duplicated"});
        }
        return res.status(400).json({error_msg: `400`, error_code: "model add Error"});
    }
...
exports.addHelpCenterAnnouncement = async (req, res) => {
    ...
    console.log(announcementObject.model)
